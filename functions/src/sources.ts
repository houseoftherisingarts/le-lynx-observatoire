...
export interface FeedSource {
  ...
  /** Un fil dedie au dossier : tout ce qu'il publie compte. */
  dedicated: boolean;
}
...
export function matchesDossier(item: FeedItem): boolean {
  if (item.source.dedicated) return true;
  const hay = `${item.title} ${item.description}`.toLowerCase();
  return KEYWORDS.some((k) => hay.includes(k));
}

/** Ne garde que ce qui est paru dans la fenetre demandee. */
export function isRecent(item: FeedItem, days: number): boolean {
...
async function fetchOne(source: FeedSource, timeoutMs = 15000): Promise<FeedItem[]> {
...
export async function collectFeedItems(days = 45): Promise<FeedItem[]> {
  const batches = await Promise.all(FEEDS.map((f) => fetchOne(f)));
  ...
  for (const items of batches) {
    for (const item of items) {
      ...
      if (!matchesDossier(item)) continue;
      if (!isRecent(item, days)) continue;
