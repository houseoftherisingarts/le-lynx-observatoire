...
const cas = [];
const verifier = (nom, promesse) => cas.push({ nom, promesse });
...
verifier('un membre ne lit pas le registre des signalements', assertFails(getDoc(doc(alex, 'projectSubmissions', 'x'))));

// --- Verdict ----------------------------------------------------------------------
let echecs = 0;
for (const { nom, promesse } of cas) {
  try {
    await promesse;
    console.log(`  ok   ${nom}`);
  } catch (e) {
